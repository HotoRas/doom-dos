...
void R_VideoErase(int y, int offset);
...
void HUlib_eraseTextLine(hu_textline_t *l)
{
  ...
  // and the text must either need updating or refreshing (because of
  // a recent change back from the automap)
  ...
  int startchar, boolean *on)
{
  ...
  s->laston = true;
  s->cl = 0;
  for (i=0;i<h;i++) HUlib_initTextLine(&s->l[i], x,
    y - i*(SHORT(font[0]->height)+1), font, startchar);
