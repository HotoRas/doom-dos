...
void A_BossDeath (mobj_t *mo)
{
	...
	else
	{
		...
		if (mo->type == MT_BRUISER && gameepisode != 1)
			return;			// bruisers aren't bosses on other episodes
	}
//
...
//
	for (i=0 ; i<MAXPLAYERS ; i++)
		if (playeringame[i] && players[i].health > 0)
			break;
	if (i==MAXPLAYERS)
		return;		// no one left alive, so don't end game
...
void A_SpawnSound (mobj_t *mo)		// travelling cube sound
{
