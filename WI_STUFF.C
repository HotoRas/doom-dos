...
void WI_drawLF(void)
{
  ...
  V_DrawPatch((SCREENWIDTH - SHORT(finished->width))/2, y,
    FB, finished);
...
void WI_drawEL(void)
{
  ...
  // draw "Entering"
  V_DrawPatch((SCREENWIDTH - SHORT(entering->width))/2, y,
    FB, entering);
...
void WI_drawOnLnode(int n, patch_t *c[])
{
  ...
  if (fits && i<2)
  {
    V_DrawPatch(lnodes[wbs->epsd][n].x, lnodes[wbs->epsd][n].y,
      FB, c[i]);
  } else printf("Could not place patch on level %d", n+1); // DEBUG
...
void WI_updateAnimatedBack(void)
{
  ...
  for (i=0;i<NUMANIMS[wbs->epsd];i++)
  {
	...
	case ANIM_RANDOM:
	  ...
	  break;
	case ANIM_LEVEL:  // gawd-awful hack for level anims
...
int WI_drawNum(int x, int y, int n, int digits)
{
  ...
  if (digits < 0)
  {
    if (!n) digits = 1; // make variable-length 0's 1 digit long
	else
...
void WI_drawTime(int x, int y, int t)
{
  ...
  if (t <= 61*59)
  {
    ...
    do
    {
      ...
      div *= 60;
      if (div==60 || t / div) V_DrawPatch(x, y, FB, colon); // draw
    } while (t / div);
  }
  else
...
void WI_drawShowNextLoc(void)
{
  ...
  if (!commercial)
  {
	  ...
	  for (i=0 ; i<=last ; i++)
		WI_drawOnLnode(i, &splat); // draw a splat on taken cities.
	  if (wbs->didsecret) WI_drawOnLnode(8, &splat); // splat the secret level
	  if (snl_pointeron) WI_drawOnLnode(wbs->next, yah); // draw flashing ptr
  }

  if (!commercial || wbs->next != 30)
	  WI_drawEL();  // draws which level you're entering..
...
int WI_fragSum(int playernum)
{
  ...
  for (i=0 ; i<MAXPLAYERS ; i++)
    if (playeringame[i] && i!=playernum) frags += plrs[playernum].frags[i];
...
void WI_drawNetgameStats(void)
{
  ...
  V_DrawPatch(NG_STATSX+3*NG_SPACINGX-SHORT(secret->width), NG_STATSY, FB,
    secret);
  if (dofrags) V_DrawPatch(NG_STATSX+4*NG_SPACINGX-SHORT(frags->width),
    NG_STATSY, FB, frags);
...
void WI_Ticker(void)
{
  ...
  if (bcnt == 1)
  {
	...
	S_ChangeMusic(mus_inter, true); // intermission music
  }
...
*/
  ...
  if (commercial)
  {
	  ...
	  level_start = GetNumForName("CLVSTART")+1;
	  NUMCMAPS = W_GetNumForName("CLVEND") - level_start + 1;
	  lnames = (patch_t **) Z_Malloc(sizeof(patch_t*) * NUMCMAPS,
		PU_STATIC, 0);

	  for (i=0 ; i<NUMCMAPS ; i++)
	  {
		lnames[i] = W_CacheLumpNum(level_start + i, PU_STATIC,);
	  }
#endif
  ...
  wiminus = W_CacheLumpName("WIMINUS", PU_STATIC); // minus sign
