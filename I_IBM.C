...
byte            *screen; 
byte            *currentscreen, *destscreen, *destview; 
...
{ 
	static  int             olddirtybox[4]; 
	static  int             volddirtybox[4]; 
	int                             updatebox[4]; 
 
// combine current dirtybox with olddirtyboxes 
	currentscreen = destscreen; 
 
	updatebox[BOXTOP] = dirtybox[BOXTOP] > volddirtybox[BOXTOP] 
		? dirtybox[BOXTOP] : volddirtybox[BOXTOP]; 
	updatebox[BOXRIGHT] = dirtybox[BOXRIGHT] > volddirtybox[BOXRIGHT] 
		? dirtybox[BOXRIGHT] : volddirtybox[BOXRIGHT]; 
	updatebox[BOXBOTTOM] = dirtybox[BOXBOTTOM] < volddirtybox[BOXBOTTOM] 
		? dirtybox[BOXBOTTOM] : volddirtybox[BOXBOTTOM]; 
	updatebox[BOXLEFT] = dirtybox[BOXLEFT] < volddirtybox[BOXLEFT] 
		? dirtybox[BOXLEFT] : volddirtybox[BOXLEFT]; 
 
	updatebox[BOXTOP] = updatebox[BOXTOP] > olddirtybox[BOXTOP] 
		? updatebox[BOXTOP] : olddirtybox[BOXTOP]; 
	updatebox[BOXRIGHT] = updatebox[BOXRIGHT] > olddirtybox[BOXRIGHT] 
		? updatebox[BOXRIGHT] : olddirtybox[BOXRIGHT]; 
	updatebox[BOXBOTTOM] = updatebox[BOXBOTTOM] < olddirtybox[BOXBOTTOM] 
		? updatebox[BOXBOTTOM] : olddirtybox[BOXBOTTOM]; 
	updatebox[BOXLEFT] = updatebox[BOXLEFT] < olddirtybox[BOXLEFT] 
		? updatebox[BOXLEFT] : olddirtybox[BOXLEFT]; 
 
// save this drawing into olddirtybox 
	memcpy (volddirtybox, olddirtybox, sizeof(volddirtybox)); 
	memcpy (olddirtybox, dirtybox, sizeof(olddirtybox)); 
 
// update other drawing 
...
// 
	...
	{ 
		...
		if (tics > 20) 
			tics = 20; 
		outpw (SC_INDEX, SC_MAPMASK + 256); 
 
		for (i=0 ; i<tics ; i++) 
			destscreen[ 199*80 + i] = 0xff; 
		for ( ; i<20 ; i++) 
			destscreen[ 199*80 + i] = 0x0; 
	} 
...
// 
	_outbyte (SC_INDEX,SC_MEMMODE); 
	_outbyte (SC_INDEX+1,(_inbyte(SC_INDEX+1)&~8)|4); 
 
// 
// turn off odd/even and set write mode 0 
// 
	_outbyte (GC_INDEX,GC_MODE); 
	_outbyte (GC_INDEX+1,_inbyte(GC_INDEX+1)&~0x13); 
 
// 
// turn off chain 4 
// 
	_outbyte (GC_INDEX,GC_MISCELLANEOUS); 
	_outbyte (GC_INDEX+1,_inbyte(GC_INDEX+1)&~2); 
 
// 
// clear the entire buffer space, because int 10h only did 16 k / plane 
// 
	_outhword (SC_INDEX,SC_MAPMASK+15*256); 
	memset (screen,0,0x10000); 
 
// 
// change CRTC scanning from doubleword to byte mode, allowing >64k scans 
// 
	_outbyte (CRTC_INDEX,CRTC_UNDERLINE); 
	_outbyte (CRTC_INDEX+1,_inbyte(CRTC_INDEX+1)&~0x40); 
 
	_outbyte (CRTC_INDEX,CRTC_MODE); 
	_outbyte (CRTC_INDEX+1,_inbyte(CRTC_INDEX+1)|0x40); 
 
	_outbyte (GC_INDEX,GC_READMAP); 
