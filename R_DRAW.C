...
#if (!defined(USEASM) && !defined(__WATCOMC__) && !defined(__i386) && !defined(__m68k))
...
{ 
	int			count; 
	byte		*source, *dest, *colormap; 
	unsigned	frac, fracstep,fracstep2,fracstep3,fracstep4;	 
...
#ifdef __WATCOMC__ 
{ 
// copy the screen into VGA page 4 
	int	p; 
	...
	{ 
		...
		outp (SC_INDEX+1,1<<p); 
		for (x=0 ; x<SCREENWIDTH*(SCREENHEIGHT-SBARHEIGHT)/4 ; x++) 
			dest[x] = src[x<<2]; 
	} 
} 
#endif 
